...
async function checkRateLimit(env: Env, ip: string): Promise<boolean> {
  ...
  try {
    ...
    await env.DB.prepare("DELETE FROM rate_limits WHERE reset_time < ?")
      .bind(now)
      .run()
    ...
    if (!record) {
      await env.DB.prepare(
        ...
        .bind(ip, now + RATE_LIMIT_WINDOW_MS)
        .run()
      return true
    }
...
async function handlePush(
  ...
  if (attempts.length > 0) {
    const statements = attempts.map((attempt) => {
      const metadataStr =
        typeof attempt.metadata === "string"
          ? attempt.metadata
  ...
  return new Response(
    JSON.stringify({ success: true, count: attempts.length }),
    {
      headers: corsHeaders,
    }
  )

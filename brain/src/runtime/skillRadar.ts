...
export function computeCategoryAverages(
  ...
  records.forEach((r) => {
    if (
      r.testId === "reaction-time" ||
      r.testId === "f1-lights" ||
      r.testId === "sound-reaction" ||
      r.testId === "aim-coordination"
    ) {
      scores.reaction.push(r.percentile)
    } else if (
      r.testId === "sequence-memory" ||
      r.testId === "number-memory" ||
      r.testId === "visual-pattern" ||
      r.testId === "dual-n-back" ||
      r.testId === "verbal-memory"
    ) {
      scores.memory.push(r.percentile)
    } else if (
      r.testId === "choice-reaction" ||
      r.testId === "pattern-reasoning" ||
      r.testId === "spatial-orientation" ||
      r.testId === "decision-speed" ||
      r.testId === "planning" ||
      r.testId === "prioritization" ||
      r.testId === "gauntlet"
    ) {
      scores.processing.push(r.percentile)
    } else if (
      r.testId === "aim-trainer" ||
      r.testId === "mouse-accuracy" ||
      r.testId === "flick-trainer"
    ) {
...
export function calculateBbiScore(avg: CognitiveAverages): number {
  ...
  const averagePercentile =
    activePercentiles.reduce((a, b) => a + b, 0) / activePercentiles.length
...
export function getRadarCoordinates(
  ...
  const keys: Array<keyof CognitiveAverages> = [
    "reaction",
    "memory",
    "processing",
    "precision",

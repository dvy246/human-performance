...
describe('determinePersona', () => {
  it('returns Rapid Reactor when reaction is strongest', () => {
    const avg: CognitiveAverages = { reaction: 90, memory: 50, processing: 50, precision: 50, focus: 50, stamina: 50 };
  ...
  it('returns Pattern Hunter when memory is strongest', () => {
    const avg: CognitiveAverages = { reaction: 50, memory: 90, processing: 50, precision: 50, focus: 50, stamina: 50 };
    const persona = determinePersona(avg);
    expect(persona.title).toBe('Pattern Hunter');
  });

  it('returns Choice Strategist when processing is strongest', () => {
    const avg: CognitiveAverages = { reaction: 50, memory: 50, processing: 90, precision: 50, focus: 50, stamina: 50 };
    const persona = determinePersona(avg);
    expect(persona.title).toBe('Choice Strategist');
  });

  it('returns Precision Targeter when precision is strongest', () => {
    const avg: CognitiveAverages = { reaction: 50, memory: 50, processing: 50, precision: 90, focus: 50, stamina: 50 };
    const persona = determinePersona(avg);
    expect(persona.title).toBe('Precision Targeter');
  });

  it('returns Focus Guardian when focus is strongest', () => {
    const avg: CognitiveAverages = { reaction: 50, memory: 50, processing: 50, precision: 50, focus: 90, stamina: 50 };
    const persona = determinePersona(avg);
    expect(persona.title).toBe('Focus Guardian');
  });

  it('returns Stamina Specialist when stamina is strongest', () => {
    const avg: CognitiveAverages = { reaction: 50, memory: 50, processing: 50, precision: 50, focus: 50, stamina: 90 };
    const persona = determinePersona(avg);
    expect(persona.title).toBe('Stamina Specialist');
  });

  it('returns first persona (Rapid Reactor) on all zeros', () => {
    const avg: CognitiveAverages = { reaction: 0, memory: 0, processing: 0, precision: 0, focus: 0, stamina: 0 };
    const persona = determinePersona(avg);
    expect(persona.title).toBe('Rapid Reactor');
  });

  it('returns first persona on tie (reaction)', () => {
    const avg: CognitiveAverages = { reaction: 80, memory: 80, processing: 50, precision: 50, focus: 50, stamina: 50 };
    const persona = determinePersona(avg);
    expect(persona.title).toBe('Rapid Reactor');
  });
  ...
  it('PERSONAS covers all 6 categories', () => {
    ...
    expect(keys).toEqual(['focus', 'memory', 'precision', 'processing', 'reaction', 'stamina']);
  });
});
...
describe('getAdaptiveRecommendations', () => {
  it('returns 2 recommendations', () => {
    const avg: CognitiveAverages = { reaction: 50, memory: 50, processing: 50, precision: 50, focus: 50, stamina: 50 };
    const recs = getAdaptiveRecommendations(avg);
    expect(recs).toHaveLength(2);
  });

  it('recommends lowest categories first', () => {
    const avg: CognitiveAverages = { reaction: 90, memory: 80, processing: 70, precision: 60, focus: 50, stamina: 40 };
    const recs = getAdaptiveRecommendations(avg);
  ...
  it('text contains category name', () => {
    const avg: CognitiveAverages = { reaction: 1, memory: 50, processing: 50, precision: 50, focus: 50, stamina: 50 };
    const recs = getAdaptiveRecommendations(avg);

...
export default function CognitiveProfile() {
  ...
  const exportHistoryToCSV = () => {
    ...
    const rows = history.map(r => [
      r.id, r.testId, r.category,
      new Date(r.timestamp).toISOString(),
      r.rawScore, r.percentile,
      r.synced ? 'true' : 'false'
    ]);
  ...
  const handleResetStats = () => {
    if (window.confirm('Are you sure you want to delete ALL local stats? This action cannot be undone.')) {
    ...
    <div className="w-full flex flex-col gap-8">
      ...
      <div className="flex items-start gap-4">
        ...
        <button
          ...
          className="shrink-0 px-3 py-2 rounded-lg border border-red-500/30 bg-red-500/5 text-red-400 hover:bg-red-500/15 text-[10px] font-mono uppercase tracking-wider transition-standard active:scale-95 cursor-pointer"
      ...
      <div className="bg-subtle border border-card-border p-4 rounded-xl text-xs text-muted flex items-start gap-3 leading-relaxed shadow-sm">
        ...
        <div>
          ...
          <a href="/terms" className="text-accent hover:underline font-medium">Terms</a>{' '}
          and{' '}
          <a href="/methodology" className="text-accent hover:underline font-medium">Methodology</a>.
        </div>
      </div>
      ...
      <section>
        <div className="flex items-center gap-2 mb-4">
          <span className="text-xs font-mono uppercase tracking-widest text-muted">Personal Records</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 rounded-xl border border-card-border bg-card p-6 flex flex-col items-center shadow-lg relative overflow-hidden">
          <span className="absolute top-4 left-4 text-[10px] font-mono text-muted uppercase tracking-widest">Skill Radar</span>
      ...
      <section>
        <div className="flex items-center gap-2 mb-4">
          <span className="text-xs font-mono uppercase tracking-widest text-muted">Performance Trends</span>
          <div className="h-px flex-1 bg-card-border/40"></div>
        </div>
        <MultiTrendChart />
      </section>
      ...
      <div className="rounded-xl border border-card-border bg-card p-5 flex flex-col gap-4 shadow">
        <div className="flex justify-between items-center flex-wrap gap-2">
          <span className="text-xs font-mono uppercase tracking-widest text-muted">Per-Test Timeline</span>
          <select
            value={graphTestId}
      ...
      <section>
        <div className="flex items-center gap-2 mb-4">
          <span className="text-xs font-mono uppercase tracking-widest text-muted">Test History</span>
          <div className="h-px flex-1 bg-card-border/40"></div>
        </div>
        <div className="rounded-xl border border-card-border bg-card p-6 shadow-lg flex flex-col gap-6">
      ...
      <details className="group rounded-xl border border-card-border bg-card p-5 shadow">
        <summary className="flex items-center justify-between cursor-pointer text-xs font-mono uppercase tracking-widest text-muted list-none">
          <span>Hardware & Browser Diagnostics</span>
          <span className="text-[10px] text-accent group-open:hidden">Show details</span>
          <span className="text-[10px] text-accent hidden group-open:inline">Hide details</span>
        </summary>
        <div className="mt-5 grid grid-cols-1 md:grid-cols-2 gap-6 font-mono text-xs">
          ...
          <div className="flex flex-col gap-3 justify-center">
            <h4 className="text-xs font-bold text-foreground uppercase tracking-wider mb-1">Calibration Alerts</h4>
      ...
      <div className="flex justify-end">
        <button
          onClick={handleResetStats}
          className="px-4 py-2 rounded-lg border border-red-500/30 bg-red-500/5 text-red-400 hover:bg-red-500/15 text-[10px] font-mono uppercase tracking-wider transition-standard active:scale-95 cursor-pointer"
        >

...
const TEST_CODES: Record<string, string> = {
  "reaction-time": "RT-001",
  "f1-lights": "RT-002",
  "sound-reaction": "RT-003",
  "choice-reaction": "RT-004",
  "go-no-go": "FC-001",
  "click-speed": "ST-001",
  "aim-trainer": "MP-001",
  "sequence-memory": "MM-001",
  "number-memory": "MM-002",
  "visual-pattern": "MM-003",
  "dual-n-back": "MM-004",
  "verbal-memory": "MM-005",
  stroop: "FA-001",
  "tmt-partA": "FA-002",
  "tmt-partB": "FA-003",
  "focus-challenge": "FA-004",
  "pattern-reasoning": "RF-001",
  "spatial-orientation": "RF-002",
  "decision-speed": "RF-003",
  planning: "EF-001",
  prioritization: "EF-002",
  "mouse-accuracy": "MP-002",
  "flick-trainer": "MP-003",
  gauntlet: "FL-001",
}
...
const CATEGORIES: Record<
  string,
  { label: string; color: string; tests: string[] }
> = {
  reaction: {
    label: "Reaction & Reflexes",
    color: "#ef4444",
  ...
  precision: {
    ...
    color: "#10b981",
    tests: ["aim-trainer", "mouse-accuracy", "flick-trainer"],
  },
  focus: {
    ...
    <div className="flex flex-col gap-6">
      {Object.entries(CATEGORIES).map(([catKey, cat]) => {
        const catTests = cat.tests
          .map((t) => summaries.get(t))
          .filter(Boolean) as TestSummary[]
        ...
        return (
          <div
            key={catKey}
            className="overflow-hidden rounded-xl border border-card-border bg-card shadow"
          >
            <button
              onClick={() => toggleCategory(catKey)}
              className="flex w-full cursor-pointer items-center justify-between border-0 bg-transparent p-4 text-left transition-colors outline-none hover:bg-subtle/50"
            >
              <div className="flex items-center gap-3">
                ...
                <span className="rounded bg-subtle px-2 py-0.5 font-mono text-[10px] text-muted">
              ...
              <div className="grid grid-cols-1 gap-3 p-4 pt-0 sm:grid-cols-2 lg:grid-cols-3">
                {cat.tests.map((testId) => {
                  ...
                  return (
                    <div
                      key={testId}
                      className={`transition-standard flex flex-col gap-2 rounded-lg border p-4 ${
                        attempted
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        {attempted ? (
                          <span className="rounded border border-accent/20 bg-accent/10 px-1.5 py-0.5 font-mono text-[9px] text-accent">
                            {formatTopPercentile(
                              summary.lastPercentile || 0,
                              isLowerBetter(testId)
                            )}
                          </span>
                        ) : (
                          <span className="rounded border border-card-border/60 bg-subtle px-1.5 py-0.5 font-mono text-[9px] text-muted">
                            Not attempted
                          </span>
                        )}
                      </div>
                      ...
                      {attempted ? (
                          ...
                          <div className="flex justify-between text-muted">
                            ...
                            <span className="font-bold text-foreground">
                              {summary.personalBest !== null
                                ? formatScore(testId, summary.personalBest)
                                : "--"}
                            </span>
                          </div>
                          <div className="flex justify-between text-muted">
                            ...
                            <span className="text-foreground">
                              {summary.lastScore !== null
                                ? formatScore(testId, summary.lastScore)
                                : "--"}
                            </span>
                          </div>
                          <div className="flex justify-between text-muted">
                        ...
                        <p className="text-[11px] text-muted italic">
                          Complete this test to see your stats.
                        </p>
                      )}

                      {related.length > 0 && (
                        <div className="mt-1 flex flex-wrap gap-1.5 border-t border-card-border/40 pt-2">
                          <span className="mr-1 font-mono text-[9px] text-muted uppercase">
                            Related:
                          </span>
                          {related.map((r) => (
                            <a

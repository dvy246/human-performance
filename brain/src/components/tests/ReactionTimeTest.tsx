...
function ReactionTimeTest() {
  ...
  useEffect(() => {
    ...
    dataLayer
      .getPersonalBest("reaction-time", "lower")
      .then((pb) => {
  ...
  const renderSVGChart = (userScore: number) => {
    ...
    const userExponent =
      -Math.pow(clampedUserScore - mean, 2) / (2 * Math.pow(stdDev, 2))
    ...
    return (
      ...
      >
        ...
        {/* User Marker Label */}
        <g
          transform={`translate(${Math.max(45, Math.min(width - 55, userX))}, ${userY - 18})`}
        >
          <rect
            x="-40"
            y="-12"
  ...
  const copyChallengeLink = () => {
    ...
    const average = Math.round(
      attempts.reduce((a, b) => a + b, 0) / totalAttempts.current
    )
  ...
  return (
    <div className="relative mx-auto flex w-full max-w-2xl flex-col gap-8">
      ...
      <div
        ...
        className={`transition-standard flex min-h-[380px] w-full flex-col items-center justify-center rounded-xl p-8 text-center outline-none select-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-4 focus-visible:ring-offset-background ${
          gameState === "waiting"
            ? "border border-rose-700 bg-rose-900/90 text-white shadow-2xl shadow-rose-950/20"
            : gameState === "ready"
              ? "border border-emerald-500 bg-emerald-600/90 text-white shadow-2xl shadow-emerald-950/20"
              : gameState === "abort"
                ? "border border-rose-900/50 bg-rose-950/40 text-rose-200"
                : "border border-card-border bg-card text-foreground/80 hover:border-muted"
        }`}
      >
        {gameState === "idle" && (
          <div
        ...
        {gameState === "waiting" && (
          <div className="relative flex flex-col items-center gap-4">
            <span className="absolute top-2 right-2 rounded bg-white/10 px-2 py-0.5 font-mono text-[10px] text-white/70 uppercase">
              WAITING
            </span>
            ...
            <p className="font-mono text-xs text-white/60 uppercase">
              {t("test.attempts")} {attempts.length + 1} /{" "}
        ...
        {gameState === "ready" && (
          <div className="relative flex flex-col items-center gap-2">
            ...
            <h2 className="text-6xl font-extrabold tracking-tighter text-white drop-shadow-md select-none">
              {t("rt.click")}
            </h2>
            <span className="font-mono text-sm text-white/80 uppercase">
              {t("rt.green_now")}
            </span>
          </div>
        )}
        ...
        {gameState === "attempt-result" && (
          <div className="flex flex-col items-center gap-4">
            <span className="font-mono text-xs text-muted uppercase">
              {t("test.attempts")} {attempts.length} {t("rt.finished")}
            </span>
        ...
        {gameState === "result" && attempts.length >= totalAttempts.current && (
          <div className="flex w-full flex-col items-center gap-6 py-4">
            <div className="flex flex-col items-center gap-1">
              ...
              <span className="mt-1 font-mono text-xs text-muted uppercase">
                {t("rt.final_avg")}
              </span>
              <div className="font-mono text-5xl font-extrabold tracking-tight text-foreground">
                {finalAverage} ms
              </div>
              <span className="text-sm font-medium text-accent">
                {formatTopPercentile(
                  lookupPercentile("reaction-time", finalAverage!, true),
                  true
                )}{" "}
                {t("rt.globally")}
              </span>
            ...
            {/* SVG curve overlay */}
            <div className="my-2 w-full border-t border-b border-card-border py-4">
            ...
            <div className="grid w-full max-w-md grid-cols-3 gap-8 text-center">
              ...
              <div>
                ...
                <div className="flex justify-center gap-1 font-mono text-xs text-foreground">
                  {attempts.map((a, idx) => (
      ...
      {calibration && (
        <div className="flex items-center justify-center gap-2 font-mono text-xs text-muted">
          <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-500" />
          <span>
            {t("rt.monitor_calibrated")} {calibration.hz}Hz{" "}
            {t("rt.refresh_rate")} | {t("rt.expected_lag")} ~
            {calibration.expectedLagMs}ms
          </span>
        </div>
      )}
    </div>

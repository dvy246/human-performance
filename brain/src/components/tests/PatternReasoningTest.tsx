...
function generatePatternQuestion(difficulty: string): GeneratedQuestion {
  ...
  while (c3 === c1 || c3 === c2)
    c3 = Math.floor(Math.random() * colorIdxs.length)
  ...
  const sequence: React.ReactNode[] = p.seq.map((ci, i) => (
    <SvgCircle key={i} color={SVG_COLORS[ci]} size={40} />
  ))
  sequence.push(<SvgQuestionMark key="q" size={40} />)

  const correctColor = SVG_COLORS[p.answer]
  const options: React.ReactNode[] = [
    <SvgCircle key="c0" color={correctColor} size={32} />,
  ]
  ...
  while (options.length < 4) {
    ...
    if (!usedColors.has(rc)) {
      usedColors.add(rc)
      options.push(
        <SvgCircle key={options.length} color={SVG_COLORS[rc]} size={32} />
      )
    }
  }
  // Shuffle
...
function generateMatrixQuestion(difficulty: string): GeneratedQuestion {
  ...
  while (otherShapeIdx === shapeIdx)
    otherShapeIdx = Math.floor(Math.random() * SHAPES.length)

  const colorIdx = Math.floor(Math.random() * SVG_COLORS.length)
  let otherColorIdx = Math.floor(Math.random() * SVG_COLORS.length)
  while (otherColorIdx === colorIdx)
    otherColorIdx = Math.floor(Math.random() * SVG_COLORS.length)
  ...
  if (ruleType === 0) {
    ...
    distractors = [
      ...
      <Shape2
        color={SVG_COLORS[(otherColorIdx + 1) % SVG_COLORS.length]}
        size={36}
      />,
    ]
  } else if (ruleType === 1) {
    ...
    while (thirdShapeIdx === shapeIdx || thirdShapeIdx === otherShapeIdx)
      thirdShapeIdx = Math.floor(Math.random() * SHAPES.length)
  ...
  return {
    ...
    correctIdx: options.findIndex((o) => {
      ...
      return (
        el.props?.color === correctEl.props?.color &&
        (el.type as Function) === (correctEl.type as Function)
      )
    }),
  }
}
...
function generateSequenceQuestion(difficulty: string): GeneratedQuestion {
  ...
  for (const a of otherAngles) {
    options.push(
      <SeqShape color={SVG_COLORS[colorIdx]} rotation={a} size={32} />
    )
  }
  // Shuffle
  for (let i = options.length - 1; i > 0; i--) {
...
function generateAnalogyQuestion(difficulty: string): GeneratedQuestion {
  ...
  while (ci3 === ci1 || ci3 === ci2)
    ci3 = Math.floor(Math.random() * SVG_COLORS.length)
  ...
  const correctOption = {
    shapeIdx: t.correctShapeIdx,
    colorIdx: t.correctColorIdx,
    size: 44,
  }

  const options: { shapeIdx: number; colorIdx: number; size: number }[] = [
    correctOption,
  ]
  ...
  const correctIdx = options.findIndex(
    (o) =>
      o.shapeIdx === correctOption.shapeIdx &&
      o.colorIdx === correctOption.colorIdx
  )
  ...
  const analogyDisplay = {
    a: (
      <AShape
        color={getColor(analogyData.a.colorIdx)}
        size={analogyData.a.size}
      />
    ),
    b: (
      <BShape
        color={getColor(analogyData.b.colorIdx)}
        size={analogyData.b.size}
      />
    ),
    c: (
      <CShape
        color={getColor(analogyData.c.colorIdx)}
        size={analogyData.c.size}
      />
    ),
  }
...
function PatternReasoningTest() {
  ...
  useEffect(() => {
    let mounted = true
    dataLayer
      .getPersonalBest("pattern-reasoning", "higher")
      .then((pb) => {
  ...
  const startTest = (mode: GameMode, config?: GameConfig) => {
    ...
    for (let i = 0; i < questionCount.current; i++) {
      if (mode === "pattern")
        questionList.push(generatePatternQuestion(difficulty))
      else if (mode === "matrix")
        questionList.push(generateMatrixQuestion(difficulty))
      else if (mode === "sequence")
        questionList.push(generateSequenceQuestion(difficulty))
  ...
  const finishTest = async (finalScore: number) => {
    ...
    const percentile = Math.round(
      lookupPercentile("pattern-reasoning", finalScore)
    )
    ...
    try {
      const card = await generateShareCard(
        `Reasoning: ${MODE_TITLES[currentMode]}`,
        `${finalScore} Pts`,
        percentile
      )
      setShareImage(card)
  ...
  return (
    <div className="mx-auto flex w-full max-w-lg flex-col gap-6 select-none">
          ...
          <div className="grid grid-cols-2 gap-3 text-left">
            {MODES.map((m) => (
              <button
                key={m}
                onClick={() =>
                  startTest(m, loadTestConfig("pattern-reasoning"))
                }
                className="transition-standard group flex cursor-pointer flex-col gap-1 rounded-xl border border-card-border/80 bg-subtle p-4 text-left hover:border-accent hover:bg-card active:scale-98"
              >
      ...
      {gameState === "running" && (
        <div className="animate-fade-in relative flex min-h-[360px] flex-col items-center justify-between overflow-hidden rounded-xl border border-card-border bg-card p-8 shadow-lg">
          <button
          ...
          <div className="mb-6 flex w-full items-center justify-between border-b border-card-border/40 pb-3 font-mono text-xs text-muted">
            <span className="font-semibold tracking-wider text-accent uppercase">
              {MODE_TITLES[currentMode]}
            </span>
            <span>
              Q {currentIdx + 1} / {questionCount.current}
            </span>
            <span>SCORE: {score} Pts</span>
          </div>

          <div className="my-4 flex min-h-[140px] w-full flex-1 flex-col items-center justify-center text-center">
            <span className="mb-4 font-mono text-xs text-secondary">
              {q?.instructions}
            </span>

            {currentMode === "pattern" && !!q && q.sequence && (
              <div className="flex flex-wrap items-center justify-center gap-3 rounded-xl border border-card-border/40 bg-subtle px-6 py-4">
              ...
              <div className="flex flex-wrap items-center justify-center gap-3 rounded-xl border border-card-border/40 bg-subtle px-6 py-4">
                {q.sequence.map((item, i) => (
                  <div key={i} className="flex items-center gap-2">
                    <div className="flex min-h-[48px] min-w-[48px] items-center justify-center rounded border border-card-border/80 bg-card px-3 py-2">
                      {item}
                    </div>
                    {i < q.sequence!.length - 1 && (
            ...
            {currentMode === "matrix" && q?.matrix && (
              <div className="mx-auto grid grid-cols-2 gap-3 rounded-xl border border-card-border/40 bg-subtle p-4">
                <div className="flex h-20 w-20 items-center justify-center rounded border border-card-border/80 bg-card">
                  {q.matrix.topLeft}
                </div>
                <div className="flex h-20 w-20 items-center justify-center rounded border border-card-border/80 bg-card">
                  {q.matrix.topRight}
                ...
                <div className="flex h-20 w-20 items-center justify-center rounded border border-card-border/80 bg-accent/5 bg-card">
            ...
            {currentMode === "analogy" && q?.analogy && (
              <div className="flex flex-wrap items-center justify-center gap-3 rounded-xl border border-card-border/40 bg-subtle px-6 py-4">
                ...
                <span className="font-mono text-xs text-muted">is to</span>
                <div className="flex items-center gap-2">{q.analogy.b}</div>
                <span className="font-mono text-xs text-muted">as</span>
          ...
          <div className="grid w-full grid-cols-2 gap-4">
            {q?.options.map((option, idx) => (
              <button
                ...
                className="transition-standard flex h-16 items-center justify-center rounded-lg border border-card-border text-sm font-semibold text-foreground hover:border-accent hover:bg-subtle active:scale-97"
              >
                {option}
              </button>
            ))}
      ...
      {gameState === "result" && (
        <div className="animate-fade-in flex flex-col items-center gap-6 rounded-xl border border-card-border bg-card p-8 text-center shadow-lg">
          ...
          <div className="my-2 grid w-full max-w-sm grid-cols-2 gap-4 border-t border-b border-card-border/50 py-4 text-left">
            <div className="flex flex-col">
              ...
              <span className="text-sm font-bold text-foreground">
                {answers.filter(Boolean).length} / {questionCount.current}
              </span>
            </div>
            <div className="flex flex-col">
              <span className="font-mono text-[10px] text-muted uppercase">
              ...
              <span className="text-sm font-bold text-foreground">
                {Math.round(
                  (answers.filter(Boolean).length / questionCount.current) * 100
                )}
                %
              </span>
            </div>
            ...
            <div className="flex flex-col">
              ...
              <span className="text-sm font-bold text-foreground">
                {Math.round(
                  latencies.reduce((a, b) => a + b, 0) / questionCount.current
                )}{" "}
                ms
              </span>
            </div>
          ...
          <div className="grid w-full grid-cols-2 gap-4">
            <button
              onClick={() =>
                startTest(getNextMode(), loadTestConfig("pattern-reasoning"))
              }

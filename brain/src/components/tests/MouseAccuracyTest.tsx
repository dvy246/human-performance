...
function MouseAccuracyTest() {
  ...
  const spawnTarget = (idx: number) => {
    ...
    const sizeIdx = Math.min(
      Math.floor(idx / PER_SIZE),
      TARGET_SIZES.length - 1
    )
    const size = Math.round(
      TARGET_SIZES[sizeIdx] * (sizeMultiplier.current ?? 1.0)
    )
  ...
  const finalize = async (allOffsets: number[]) => {
    ...
    try {
      await dataLayer.saveSession({
        ...
        category: "precision",
        rawScore: score,
        percentile: lookupPercentile("mouse-accuracy", score),
        metadata: {
          avgOffsetPx: Math.round(avgOffset * 10) / 10,
          totalTargets: targetCount.current,
        },
      })
    } catch (err) {
    ...
    const card = await generateShareCard(
      "Mouse Accuracy Test",
      `${Math.round(avgOffset * 10) / 10}px avg`,
      lookupPercentile("mouse-accuracy", score)
    ).catch(() => "")
    if (!submittedRef.current) return
    setShareImage(card)
    ...
    return (
      <div className="mx-auto w-full max-w-2xl">
        <div className="rounded-xl border border-card-border bg-card p-4">
          <div className="mb-2 flex items-center justify-between font-mono text-[10px] text-muted">
            ...
            <span className="flex items-center gap-2">
              <span>
                Last offset:{" "}
                {offsets.length > 0
                  ? `${Math.round(offsets[offsets.length - 1] * 10) / 10}px`
                  : "--"}
              </span>
              <button
          ...
          >
            <div
              className="pointer-events-none absolute h-1 w-1 -translate-x-1/2 -translate-y-1/2 rounded-full bg-accent/30"
              style={{ left: target.x, top: target.y }}
            />
            <div
              className={`absolute -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-rose-400 bg-rose-500/40`}
              style={{
                left: target.x,
                top: target.y,
                width: target.size,
  ...
  const avg =
    offsets.length > 0
      ? Math.round((offsets.reduce((a, b) => a + b, 0) / offsets.length) * 10) /
        10
      : 0
  return (
    <div className="mx-auto flex w-full max-w-2xl flex-col gap-6">
      <div className="flex w-full flex-col items-center gap-4 rounded-xl border border-card-border bg-card p-8">
        ...
        <div className="grid w-full max-w-xs grid-cols-3 gap-4 text-center text-xs">
          <div>
            <div className="font-mono text-[10px] text-muted">Best</div>
            <div className="font-mono text-foreground">
              {Math.round(
                offsets.reduce((a, b) => Math.min(a, b), offsets[0]) * 10
              ) / 10}
              px
            </div>
          </div>
          <div>
            <div className="font-mono text-[10px] text-muted">Worst</div>
            <div className="font-mono text-foreground">
              {Math.round(
                offsets.reduce((a, b) => Math.max(a, b), offsets[0]) * 10
              ) / 10}
              px
            </div>
          </div>
          <div>
            <div className="font-mono text-[10px] text-muted">Targets</div>
            <div className="font-mono text-foreground">
              {targetCount.current}
            </div>
          </div>
        ...
        {shareImage && (
          <a
            href={shareImage}
            download="cogniarena-mouse-accuracy.png"
        ...
        <SocialShare
          ...
          scoreLabel={`${avg}px avg`}
          testName="Mouse Accuracy Test"
        />
        <button
          onClick={() => setPhase("intro")}

...
function AimCoordinationTest() {
  ...
  useEffect(() => {
    ...
    dataLayer
      .getPersonalBest("aim-coordination", "lower")
      .then((pb) => {
  ...
  const finalizeTest = async () => {
    ...
    const averageLatency =
      latenciesArr.current.length > 0
        ? Math.round(
            latenciesArr.current.reduce((a, b) => a + b, 0) /
              latenciesArr.current.length
          )
        : 0
    ...
    try {
      const card = await generateShareCard(
        ...
        lookupPercentile("aim-coordination", averageLatency, true)
      )
      setShareImage(card)
    } catch (err) {
  ...
  return (
    <div className="relative mx-auto flex w-full max-w-2xl flex-col gap-8">
      ...
      {challengeScore && gameState !== "result" && (
        <div className="flex items-center justify-between rounded-lg border border-green-500/30 bg-green-500/10 p-4 text-sm dark:border-green-900/50 dark:bg-green-950/20">
          <span className="text-foreground">
            Active Challenge: Beat your friend's coordination speed of{" "}
      ...
      {gameState === "playing" ? (
          ...
          <div className="flex items-center justify-between font-mono text-xs text-muted">
            <span>
              ...
              <strong className="text-foreground">
                {currentTargetIndex} / {targetCount.current}
              </strong>
            </span>
            <span>
              Misses: <strong className="text-error">{misses}</strong>
            </span>
          </div>
          <canvas
            ref={canvasRef}
        ...
        <div className="flex w-full flex-col items-center gap-6 rounded-xl border border-card-border bg-card p-8">
          <div className="flex flex-col items-center gap-1">
            ...
            <div className="font-mono text-5xl font-bold text-foreground">
              {Math.round(
                latencies.reduce((a, b) => a + b, 0) / targetCount.current
              )}{" "}
              ms
            </div>
          ...
          <div className="mt-2 grid w-full max-w-sm grid-cols-3 gap-6 border-t border-card-border/50 pt-4 text-center">
            <div>
              ...
              <div className="font-mono text-sm text-foreground">{misses}</div>
            </div>
            <div>
              <span className="font-mono text-[10px] text-muted uppercase">
                Hit Rate
              ...
              <div className="font-mono text-sm text-foreground">
                {clicks > 0
                  ? Math.round((targetCount.current / clicks) * 100)
                  : 0}
                %
              </div>
            </div>
        ...
        /* Idle Page */
        <div className="flex w-full flex-col items-center justify-center rounded-xl border border-card-border bg-card p-8 text-center">
          <div

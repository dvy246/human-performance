...
function SequenceMemoryTest() {
  ...
  useEffect(() => {
    let mounted = true
    dataLayer
      .getPersonalBest("sequence-memory", "higher")
      .then((pb) => {
  ...
  const handleTileClick = async (tileIdx: number) => {
    if (gameState !== "playing" || userTurnLock.current || respondedRef.current)
      return
  ...
  return (
    <div className="mx-auto flex w-full max-w-2xl flex-col gap-8">
      ...
      {/* Grid Container */}
      <div className="relative flex w-full flex-col items-center gap-6 rounded-xl border border-card-border bg-card p-8">
        ...
        <div className="flex w-full max-w-xs items-center justify-between font-mono text-xs text-muted">
          <span>
            {t("seq.level")}{" "}
        ...
        {gameState !== "result" ? (
          /* Simon Grid Layout */
          <div className="grid h-[280px] w-full max-w-[280px] grid-cols-3 gap-3.5">
            {[0, 1, 2, 3, 4, 5, 6, 7, 8].map((idx) => {
              ...
              return (
                <button
                  ...
                  className={`h-full w-full cursor-pointer rounded-lg border transition-all duration-75 outline-none select-none ${
                    isActive
                      ? "scale-95 border-accent bg-accent shadow-[0_0_20px_rgba(217,119,6,0.5)]"
          ...
          <div className="flex flex-col items-center gap-6 py-4">
            <div className="flex flex-col items-center gap-1">
              ...
              <span className="font-mono text-xs text-accent uppercase">
                {t("rt.top_globally")}{" "}
                {100 - lookupPercentile("sequence-memory", level - 1)}%{" "}
                {t("seq.population")}
              </span>
            </div>
            ...
            <div className="mt-2 grid w-full max-w-xs grid-cols-2 gap-8 border-t border-card-border/50 pt-4 text-center">
              <div>
                ...
                <div className="font-mono text-sm text-foreground">
                  {personalBest ? `${t("seq.level")} ${personalBest}` : "--"}
                </div>
              </div>
              <div>
                <span className="font-mono text-[10px] text-muted uppercase">
                  {t("seq.percentile_span")}
                </span>
                <div className="font-mono text-sm text-foreground">
                  ~
                  {Math.round(

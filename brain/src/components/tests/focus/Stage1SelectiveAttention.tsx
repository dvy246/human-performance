...
export default function Stage1SelectiveAttention({ onComplete, calibrationHz }: StageProps) {
  ...
  const handleCellClick = (idx: number) => {
    ...
    feedbackTimer.current = setTimeout(() => {
      if (trialIndex + 1 >= TOTAL_TRIALS) {
        setPhase('done');
        const accuracy = (hitCount + (correct ? 1 : 0)) / TOTAL_TRIALS;
        const avgRt = (totalRt + rt) / TOTAL_TRIALS;
  ...
  const distractors = Array.from({ length: 6 }, () => ['▲', '△', '▪', '▫', '✦', '✧'][Math.floor(Math.random() * 6)]);

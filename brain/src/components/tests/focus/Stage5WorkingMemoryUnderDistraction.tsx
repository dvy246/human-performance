...
export default function Stage5WorkingMemoryUnderDistraction({
  onComplete,
  calibrationHz,
  difficulty,
}: StageProps) {
  ...
  const [distractors, setDistractors] = useState<
    { id: number; symbol: string; row: number; col: number }[]
  >([])
  ...
  const distractorIntervalRef = useRef<
  ...
  const startDistractors = useCallback(
    (occupied: string[]) => {
      let idCount = 0
      distractorIntervalRef.current = setInterval(() => {
        ...
        const d = {
          id: idCount,
          symbol: pickRandom(DISTRACTOR_SYMBOLS),
          row: pos.row,
          col: pos.col,
        }
        ...
        st(() => {
          distractorsRef.current = distractorsRef.current.filter(
            (d2) => d2.id !== d.id
          )
          setDistractors([...distractorsRef.current])
        ...
        st(() => {
          if (levelRef.current >= MAX_LEVEL) {
            ...
            const score = Math.max(
              1,
              Math.min(100, Math.round((reached / MAX_LEVEL) * 100))
            )
            onComplete({
              stageIndex: 4,
              ...
              score,
              metrics: {
                maxLevelReached: reached,
                maxSequenceLength: Math.min(3 + reached, 9),
              },
            })
          }
        }, 1500)
        ...
        if (nextLvl > MAX_LEVEL) {
          ...
          onComplete({
            ...
            metrics: {
              ...
              maxSequenceLength: Math.min(3 + MAX_LEVEL, 9),
            },
          })
          return
        }
  ...
  const finishGame = useCallback(() => {
    ...
    const score = Math.max(
      1,
      Math.min(100, Math.round((maxLevelRef.current / MAX_LEVEL) * 100))
    )
    onComplete({
      stageIndex: 4,
      stageName: "WM Under Distraction",
      score,
      metrics: {
        maxLevelReached: maxLevelRef.current,
        maxSequenceLength: Math.min(3 + maxLevelRef.current, 9),
      },
    })
  }, [onComplete])
  ...
  const renderGrid = (showActive: boolean, showDistractors: boolean) => (
    <div className="grid grid-cols-3 gap-2">
      {GRID_POSITIONS.map((pos) => {
        ...
        const isActive =
          showActive &&
          activeCell?.row === pos.row &&
          activeCell?.col === pos.col
        const isUser = userSequence.find(
        ...
        return (
          <button
            ...
            className={`flex h-16 w-16 items-center justify-center rounded-xl border border-card-border text-lg transition-all duration-150 sm:h-20 sm:w-20 ${
              bg
            } ${isActive ? "scale-110 border-accent" : ""} ${
              phase === "recall"
                ? "cursor-pointer hover:border-accent hover:bg-accent/5 active:scale-95"
                : ""
            }`}
          >
            {distractor ? (
              ...
              <span
                className="text-2xl"
                style={{ color: sequence[seqIdx]?.color || "#fff" }}
              >
                ●
              </span>
            ) : null}
      ...
      <div className="flex flex-col items-center gap-6 py-8">
        ...
        <div className="text-center">
          <h3 className="mb-1 text-lg font-bold text-foreground">
            Stage 5: Working Memory Under Distraction
          </h3>
          <div className="max-w-md space-y-1 text-left text-sm text-secondary">
            <p>
              1. Watch the <strong className="text-accent">colored dots</strong>{" "}
              appear on the grid one by one.
            </p>
            <p>
              2. <strong className="text-accent">Ignore</strong> the distracting
              symbols flashing around the grid.
            </p>
            <p>
              3. When prompted, click the grid cells{" "}
              <strong className="text-accent">in the same order</strong> as the
              dots you saw.
            </p>
            <p>
              4. Get it right to advance to the next level with longer
              sequences.
            </p>
            <p className="mt-2 text-xs text-muted">
              {MAX_LEVEL} levels. Each level adds one more dot to remember.
            </p>
          </div>
        </div>
        <button
    ...
    return (
      <div className="flex flex-col items-center gap-4 py-6">
        ...
        <p className="text-sm text-secondary">
          Working Memory complete! Level {maxLevelReached}/{MAX_LEVEL}
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col items-center gap-5 py-4">
      <div className="flex items-center gap-3 font-mono text-xs text-muted">
        ...
        {phase === "encoding" && (
          <span className="animate-pulse text-warning">👀 Watch</span>
        )}
        <button
      ...
      {showHelp && (
        <div className="animate-in fade-in w-full max-w-md space-y-1 rounded-lg border border-card-border bg-panel p-3 font-mono text-xs text-muted duration-150">
          ...
          <p>
            2. <strong className="text-error">Ignore</strong> distracting
            symbols on the grid.
          </p>
          <p>
            3. When prompted, click cells{" "}
            <strong className="text-accent">in the same order</strong> as the
            dots.
          </p>
        </div>

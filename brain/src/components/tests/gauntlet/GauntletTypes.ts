...
export function getArchetype(results: GauntletStageResult[]): {
  title: string
  desc: string
} {
  const cats: Record<string, number> = {}
  results.forEach((r) => {
    cats[r.category] = (cats[r.category] || 0) + r.score
  })
  ...
  const archetypes: Record<string, { title: string; desc: string }> = {
    reaction: {
      title: "Rapid Reactor",
      desc: "Your reflexes fire before conscious thought. Sensory-motor speed is your edge.",
    },
    memory: {
      title: "Pattern Hunter",
      desc: "You encode and retrieve complex patterns effortlessly. Your memory is your weapon.",
    },
    processing: {
      title: "Choice Strategist",
      desc: "You resolve complex decisions quickly and accurately. Mental speed is your strength.",
    },
    precision: {
      title: "Precision Targeter",
      desc: "Your motor control is surgical. Fine adjustments and accuracy define you.",
    },
    focus: {
      title: "Iron Mind",
      desc: "Distractions bounce off you. Sustained concentration is your superpower.",
    },
  }
  return (
    archetypes[top] || {
      title: "Balanced",
      desc: "Well-rounded cognitive profile across all domains.",
    }
  )
}

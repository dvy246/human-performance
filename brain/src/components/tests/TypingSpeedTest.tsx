...
class TypingEngine {
  ...
  private tick() {
    ...
    if (
      ...
      (this.isWordMode && this.currentWordIdx >= this.wordCount)
    ) {
      this.endTest()
      return
    }
  ...
  endTest() {
    ...
    this.reactionDelay =
      this.firstKeyTimes.length > 0
        ? Math.round(
            this.firstKeyTimes.reduce((a, b) => a + b, 0) /
              this.firstKeyTimes.length
          )
        : 0
    this.onUpdate()
  }
  ...
  private calcBurst(): number {
    ...
    for (let i = 0; i < this.wordTimings.length - windowSize; i++) {
      const totalTime =
        this.wordTimings.slice(i, i + windowSize).reduce((a, b) => a + b, 0) /
        1000
      if (totalTime > 0) {
  ...
  handleKeydown(e: KeyboardEvent): boolean {
    ...
    if (e.key === "Escape") {
      if (this.isActive || this.submitted) {
        this.reset()
        this.onUpdate()
        return true
      }
    ...
    if (e.key === " " || e.key === "Enter") {
      ...
      if (!isCorrect) {
        ...
        this.incorrectStrokes +=
          Math.max(0, typedLen - correctCount) + Math.max(0, wordLen - typedLen)
      }
      ...
      if (this.isWordMode && this.currentWordIdx >= this.wordCount) {
        this.endTest()
        return true
      }
      this.onUpdate()
  ...
  private trackChar(expected: string, actual: string) {
    if (!this.charErrors[expected])
      this.charErrors[expected] = { wrong: 0, total: 0 }
  ...
  get stats(): StatsSnapshot {
    ...
    return {
      ...
      correct: this.correctStrokes,
      incorrect: this.incorrectStrokes,
      total: t,
      burstSpeed: this.burstSpeed || peak,
...
function interpolatePercentile(score: number): number {
  const table = (
    percentilesData as Record<string, { score: number; percentile: number }[]>
  )["typing-speed"]
...
function TypingSpeedTest() {
  ...
  const updateCursor = useCallback(() => {
    ...
    if (!cursor || !wordsEl || engine.submitted) {
      if (cursor) cursor.style.opacity = "0"
      return
    }
    const wordSpans = wordsEl.querySelectorAll<HTMLElement>("[data-word-idx]")
    const currentWordEl = wordsEl.querySelector<HTMLElement>(
      `[data-word-idx="${engine.currentWordIdx}"]`
    )
    if (!currentWordEl) {
    ...
    const charSpans =
      currentWordEl.querySelectorAll<HTMLElement>("[data-char-idx]")
      ...
      if (t) {
        import("../../runtime/share")
          .then(({ decodeChallenge }) => {
            ...
            if (payload && payload.testId === "typing-speed" && mounted)
  ...
  const pickRandomPassage = useCallback(
    (catIdx: number, excludeIdx?: number) => {
  ...
  const startTest = () => {
    ...
    const passageText = cat
      ? cat.passages[newPassageIdx % cat.passages.length].text
      : PASSAGE_CATEGORIES[0].passages[0].text
  ...
  const switchCategory = (idx: number) => {
    setCategoryIdx(idx)
    setPassageIdx(pickRandomPassage(idx))
  }
  ...
  const renderWord = (word: string, wordIdx: number) => {
    ...
    if (isPast) {
      ...
      return (
        <span
          ...
          className="tts-word tts-word-past"
          style={{
            color: allCorrect
              ? "var(--tts-past-correct)"
              : "var(--tts-past-incorrect)",
    ...
    if (isCurrent) {
      ...
      return (
        <span
          key={wordIdx}
          data-word-idx={wordIdx}
          className="tts-word tts-word-current"
        >
            ...
            if (ci === typed.length) {
              return (
                <span
                  key={ci}
                  data-char-idx={ci}
                  className="tts-char tts-char-active"
                  style={{ color: "var(--tts-faint)" }}
                >
                  {char}
                </span>
              )
          ...
          {extra.map((ch, ei) => (
            <span
              key={`e${ei}`}
              data-char-idx={word.length + ei}
              className="tts-char"
              style={{
                color: "var(--tts-incorrect)",
                background: "var(--tts-incorrect-bg)",
                borderRadius: "2px",
              }}
            >
              {ch}
    ...
    return (
      <span
        key={wordIdx}
        data-word-idx={wordIdx}
        className="tts-word tts-word-future"
        style={{ color: "var(--tts-dim)" }}
      >
        {word.split("").map((ch, ci) => (
          <span key={ci} data-char-idx={ci} className="tts-char">
  ...
  if (gameState === "result" && resultStats) {
    ...
    const maxWpm = rs.wpmSamples.reduce(
    ...
    return (
      ...
      >
        <div className="flex flex-col items-center gap-8">
          <div className="flex flex-col items-center gap-2">
            ...
            <div className="flex items-baseline gap-2">
              <span
                ...
                style={{ color: "var(--tts-text)", lineHeight: 1 }}
              >
                {rs.wpm}
              </span>
              <span
                className="text-2xl font-normal"
                style={{ color: "var(--tts-muted)" }}
              >
                wpm
              </span>
            </div>
          ...
          <div
            className="grid w-full max-w-lg grid-cols-2 gap-x-8 gap-y-4 text-center md:grid-cols-4"
            style={{
              borderTop: "1px solid var(--tts-border)",
              paddingTop: "1.5rem",
            }}
          >
            {[
          ...
          >
            <span>
              ...
              <strong
                style={{
                  color: isPb ? "var(--tts-accent)" : "var(--tts-text)",
                }}
              >
          ...
          {rs.wpmSamples.length > 1 && (
            <div className="w-full max-w-md">
              <span className="tts-label mb-2 block">Performance Timeline</span>
              <div
                style={{
                  ...
                  <path
                    d={(() => {
                      ...
                      return v
                        .map(
                          (val, i) =>
                            `${i === 0 ? "M" : "L"} ${(i / (v.length - 1)) * 300} ${80 - (val / maxWpm) * 75}`
                        )
                        .join(" ")
                    })()}
                    strokeWidth="2"
                    stroke="var(--tts-accent)"
                    fill="none"
                    strokeLinecap="round"
                  ...
                  <path
                    d={(() => {
                      ...
                      return v
                        .map(
                          (val, i) =>
                            `${i === 0 ? "M" : "L"} ${(i / (v.length - 1)) * 300} ${80 - (val / 100) * 75}`
                        )
                        .join(" ")
                    })()}
                    strokeWidth="1.5"
          ...
          {Object.keys(rs.charErrors).length > 0 && (
            <div className="w-full max-w-md">
              <span className="tts-label mb-2 block">Error Heatmap</span>
                ...
                {Object.entries(rs.charErrors)
                  ...
                  .map(([char, data]) => {
                    const intensity = Math.min(
                      1,
                      (data.wrong / Math.max(1, data.total)) * 3
                    )
                    return (
                      <span
                        key={char}
                        className="inline-flex h-7 w-7 items-center justify-center rounded font-mono text-xs"
                        style={{
                          background: `rgba(196,64,64,${0.1 + intensity * 0.4})`,
                          color: "var(--tts-text)",
          ...
          <div className="flex flex-wrap items-center justify-center gap-3">
            ...
            {shareImage && (
              <a
                href={shareImage}
                download="cogniarena-typing-score.png"
        ...
        <div className="flex flex-col items-center gap-8">
          ...
          <div className="flex items-center gap-2 font-mono">
            <button
              ...
              style={{
                background:
                  testMode === "time" ? "var(--tts-accent-bg)" : "transparent",
                color:
                  testMode === "time"
                    ? "var(--tts-accent)"
                    : "var(--tts-muted)",
            ...
            <button
              ...
              style={{
                background:
                  testMode === "words" ? "var(--tts-accent-bg)" : "transparent",
                color:
                  testMode === "words"
                    ? "var(--tts-accent)"
                    : "var(--tts-muted)",
          ...
          <div className="flex items-center gap-2 font-mono">
            {(testMode === "time" ? [15, 30, 60, 120] : [10, 25, 50, 100]).map(
              (v) => (
                <button
                  key={v}
                  onClick={() =>
                    testMode === "time"
                      ? setTimerDuration(v as TimeOption)
                      : setWordCount(v as WordOption)
                  }
                  className="tts-opt"
                  style={{
                    background: (
          ...
          <div className="flex w-full max-w-md flex-col items-center gap-3">
            ...
            <span
              ...
              style={{ color: "var(--tts-faint)" }}
            >
              Passage {passageIdx + 1} of{" "}
              {PASSAGE_CATEGORIES[categoryIdx].passages.length} &mdash;
          ...
          >
            <div className="tts-words-preview">
              {PASSAGE_CATEGORIES[categoryIdx].passages[passageIdx].text
                .split(/\s+/)
                .filter(Boolean)
                .map((w, i) => (
                  <span
        ...
        <div className="relative flex flex-col gap-6">
          ...
          <div className="flex items-center justify-center gap-10 md:gap-16">
            {[
              ...
              {
                label: testMode === "time" ? "TIME" : "WORDS",
                value:
                  testMode === "time"
                    ? `${displayTime}s`
                    : `${currentWordIdx}/${wordCount}`,
                color:
                  testMode === "time" && displayTime <= 5
                    ? "var(--tts-incorrect)"
                    : "var(--tts-text)",
              },
            ].map((s) => (
              <div
                key={s.label}
                className="flex flex-col items-center gap-0.5 font-mono"
              >
                <span className="tts-label">{s.label}</span>
                <span
          ...
          <div
            className="tts-words-outer"
            style={{
              height: "calc(1.8em * 3 + 0.5rem)",
              overflow: "hidden",
              position: "relative",
            }}
          >
            ...
            >
              ...
              {words
                .slice(
                  Math.max(0, currentWordIdx - 30),
                  Math.min(words.length, currentWordIdx + 120)
                )
                .map((word, i) => {
          ...
          <div
            className="flex items-center justify-center gap-4 font-mono text-[10px]"
            style={{ color: "var(--tts-faint)" }}
          >

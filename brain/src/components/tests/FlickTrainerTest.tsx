...
function FlickTrainerTest() {
  ...
  const finalize = async (r: { rt: number; hit: boolean }[]) => {
    ...
    try {
      await dataLayer.saveSession({
        testId: 'flick-trainer', category: 'precision', rawScore: score, percentile: lookupPercentile('flick-trainer', score),
  ...
  const renderCrosshair = () => {
    ...
    return (
      <svg width={r * 2} height={r * 2} viewBox={`0 0 ${r * 2} ${r * 2}`} className="absolute -translate-x-1/2 -translate-y-1/2 pointer-events-none" style={{ left: target.x, top: target.y }}>
        <circle cx={r} cy={r} r={r - 2} fill="#ef4444" fillOpacity={0.35} stroke="#ef4444" strokeWidth={2} />
        <line x1={r - 6} y1={r} x2={r + 6} y2={r} stroke="#ef4444" strokeWidth={1.5} />
        <line x1={r} y1={r - 6} x2={r} y2={r + 6} stroke="#ef4444" strokeWidth={1.5} />
      </svg>
    );
  };
  ...
  return (
    <div className="w-full flex flex-col gap-6 max-w-2xl mx-auto">
      <div className="w-full rounded-xl border border-card-border bg-card p-8 flex flex-col items-center gap-4">
        ...
        {shareImage && (
          <a href={shareImage} download="cogniarena-flick-trainer.png" className="flex items-center justify-center gap-2 rounded-md bg-accent hover:bg-accent-hover text-white font-semibold h-10 text-sm active:scale-[0.98] transition-standard cursor-pointer">

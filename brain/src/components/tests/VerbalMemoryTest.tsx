      ...
      <div className="w-full flex flex-col gap-8 max-w-2xl mx-auto">
        <div className="w-full rounded-xl border border-card-border bg-card p-8 flex flex-col items-center gap-6">
          ...
          <div className="text-center">
            ...
            <p className="text-zinc-400 text-sm max-w-sm mx-auto mt-2">Words appear one at a time. Remember them, then pick them out from a larger set.</p>
          </div>
          <button onClick={() => { setPhase('encoding'); setLevel(1); setMaxCorrect(0); startLevel(1); }} className="px-8 h-12 rounded-lg bg-accent hover:bg-accent-hover text-black font-bold text-sm transition-standard active:scale-95 cursor-pointer">Start Test</button>
        </div>
      </div>
    );
  }
  ...
  if (phase === 'encoding') {
    return (
      <div className="w-full max-w-2xl mx-auto">
        <div className="w-full rounded-xl border border-card-border bg-card p-8 flex flex-col items-center gap-6">
          <div className="text-[10px] text-zinc-500 font-mono uppercase tracking-wider">Level {level} · Memorize {Math.min(3 + level, 12)} words</div>
    ...
    return (
      <div className="w-full max-w-2xl mx-auto">
        <div className="w-full rounded-xl border border-card-border bg-card p-8 flex flex-col items-center gap-4">
          <div className="text-[10px] text-zinc-500 font-mono uppercase tracking-wider">Select the words you saw · {selected.length}/{wordList.length}</div>
            ...
            {options.map(w => (
              <button key={w} onClick={() => toggleWord(w)}
                className={`px-3 py-1.5 rounded-lg text-sm border transition-standard cursor-pointer ${
                  selected.includes(w) ? 'bg-accent/20 border-accent text-accent' : 'bg-subtle border-card-border text-zinc-300 hover:border-accent/50'
                }`}
              >{w}</button>
            ))}
          </div>
          <button onClick={submitRecall} disabled={selected.length === 0}
            className="px-6 h-10 rounded-lg bg-accent hover:bg-accent-hover text-black font-semibold text-sm disabled:opacity-30 transition-standard active:scale-95 cursor-pointer"
      ...
      <div className="w-full flex flex-col gap-6 max-w-2xl mx-auto">
        <div className="w-full rounded-xl border border-card-border bg-card p-8 flex flex-col items-center gap-4">
          ...
          <div className="text-center">
            ...
            <div className="text-xs text-zinc-500 font-mono mt-1">Score: {score}/100 • Max Level: {level}</div>
          </div>
          {shareImage && (
            <a href={shareImage} download="cogniarena-verbal-memory.png" className="flex items-center justify-center gap-2 rounded-md bg-accent hover:bg-accent-hover text-black font-semibold h-10 text-sm active:scale-[0.98] transition-standard">

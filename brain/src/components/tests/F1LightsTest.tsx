...
const F1LightsTest = () => {
  ...
  useEffect(() => {
    ...
    // 2. Personal Best
    dataLayer
      .getPersonalBest("f1-lights", "lower")
      .then((pb) => {
  ...
  const startTest = (config?: GameConfig) => {
    ...
    waitRange.current = {
      min: (diff.waitMin as number) || 800,
  ...
  return (
    <div className="relative mx-auto flex w-full max-w-2xl flex-col gap-8">
      ...
      {challengeScore && gameState !== "result" && (
        <div className="flex items-center justify-between rounded-lg border border-amber-900/50 bg-amber-950/20 p-4 text-sm">
          <span className="text-secondary">
            Active Challenge: Beat your friend's F1 start of{" "}
          ...
          <button
            ...
            className="font-mono text-[11px] text-muted uppercase hover:text-secondary"
          >
            {t("test.dismiss")}
          </button>
        </div>
      ...
      <div
        ...
        className={`transition-standard flex min-h-[380px] w-full flex-col items-center justify-between rounded-xl border border-card-border p-8 outline-none focus-visible:ring-2 focus-visible:ring-accent ${
          gameState === "ready"
            ? "border-emerald-500 bg-subtle shadow-2xl shadow-emerald-950/10"
            : gameState === "jump-start"
      ...
      >
        ...
        <div className="my-8 flex justify-center gap-5">
          {[1, 2, 3, 4, 5].map((col) => {
            ...
            return (
              <div
                key={col}
                className="flex w-14 flex-col items-center gap-3 rounded-md border-2 border-card-border bg-panel p-2"
              >
                {/* Light number indicator */}
                <span className="absolute -top-5 font-mono text-[10px] text-muted">
                  {col}
                </span>
                {/* Upper light pair */}
                <div
                  className={`relative h-8 w-8 rounded-full border border-black shadow-inner transition-all duration-75 ${
                    isLit
                      ? "bg-red-600 ring-2 shadow-red-500/50 ring-red-500"
                      : "bg-subtle"
                  }`}
                >
                  {isLit && (
        ...
        <div className="flex max-w-md flex-1 flex-col items-center justify-center gap-3 text-center">
          {gameState === "idle" && (
            <div
          ...
          {(gameState === "sequence" || gameState === "waiting") && (
            <h3 className="font-mono text-sm tracking-wider text-foreground uppercase dark:text-white">
              {gameState === "waiting"
                ? t("f1.lights_holding")
                : t("f1.starting_seq")}
            </h3>
          ...
          {gameState === "ready" && (
            <>
              ...
              <h2 className="animate-pulse font-mono text-5xl font-bold tracking-widest text-success uppercase select-none">
                {t("f1.go")}
              </h2>
          ...
          {gameState === "jump-start" && (
            <>
              ...
              <p className="text-xs text-muted">{t("f1.clicked_before")}</p>
              <span className="mt-2 font-mono text-xs text-error uppercase">
                {t("f1.click_restart")}
              </span>
            </>
          )}
        ...
        {/* Small stats layout */}
        {gameState === "result" ? (
          <div className="mt-4 grid w-full max-w-sm grid-cols-2 gap-8 border-t border-card-border/50 pt-4 text-center">
            <div>
          ...
          <span className="font-mono text-[10px] text-muted dark:text-muted">
            {t("f1.press_spacebar")}
          </span>
        )}
      </div>

    ...
    <div className="flex flex-col gap-2 w-full pt-4 border-t border-card-border/50">
      <div className="flex items-center justify-between text-[11px] text-zinc-500 font-mono">
        ...
        {recoveryCode ? (
          ...
          <div className="flex items-center gap-1.5 text-zinc-400 dark:text-zinc-600">
            <span className="w-1.5 h-1.5 rounded-full bg-zinc-400 dark:bg-zinc-700"></span>
      ...
      {recoveryCode ? (
        <button
          ...
          className="text-left text-[11px] text-zinc-500 dark:text-zinc-400 hover:text-foreground hover:underline transition-colors mt-0.5"
        >
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-card border border-card-border p-6 rounded-xl relative shadow-2xl">
            <button
              ...
              className="absolute right-4 top-4 text-zinc-500 hover:text-foreground font-mono text-lg p-1"
            ...
            {recoveryCode ? (
              ...
              <div className="flex flex-col gap-4">
                <div className="bg-subtle border border-card-border p-4 rounded-lg flex flex-col gap-2">
                  <span className="text-[10px] font-mono uppercase text-zinc-500">Your 6-Word Recovery Code</span>
                ...
                <div className="flex gap-3 justify-end mt-2">
                  <button
                    ...
                    className="px-3 h-8 text-[11px] font-mono uppercase border border-red-500/40 hover:border-red-500 bg-red-500/10 text-red-500 rounded active:scale-98 transition-all cursor-pointer"
                  >
                    Disconnect
                  </button>
                  <button
                    ...
                    className="px-3 h-8 text-[11px] font-mono uppercase bg-accent text-black font-bold rounded hover:bg-accent-hover active:scale-98 transition-all cursor-pointer"
                  >
                    Force Sync Now
                  </button>
                </div>
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-2 p-4 rounded-lg bg-subtle border border-card-border">
                  ...
                  <p className="text-zinc-500 text-[11px] leading-normal mb-2">
                ...
                <form onSubmit={handlePullSync} className="flex flex-col gap-2 p-4 rounded-lg bg-subtle border border-card-border">
                  ...
                  <p className="text-zinc-500 text-[11px] leading-normal mb-3">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 h-8 bg-[var(--bg-input)] border border-card-border px-3 rounded text-xs font-mono text-foreground placeholder-zinc-500 focus:outline-none focus:border-accent"
                    />
                    <button
                      type="submit"
                      disabled={syncStatus === 'syncing'}
                      className="h-8 px-4 text-[11px] font-mono uppercase bg-accent hover:bg-accent-hover text-black font-semibold rounded active:scale-98 transition-all cursor-pointer"
                    >
                      Import
                    </button>
                  </div>
            ...
            {syncStatus !== 'idle' && (
              <div className={`mt-4 p-3 rounded-lg border text-xs font-mono text-center ${
                syncStatus === 'syncing' ? 'bg-subtle border-card-border text-zinc-500' :
                syncStatus === 'success' ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-500' :

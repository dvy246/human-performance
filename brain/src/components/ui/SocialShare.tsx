...
export default function SocialShare({ testId, score, scoreLabel, testName }: SocialShareProps) {
  const [copied, setCopied] = useState(false);

  const getChallengeUrl = () => {
    if (typeof window === 'undefined') return '';
    const token = encodeChallenge({ testId, score });
    return `${window.location.origin}/tests/${testId === 'reaction-time' ? 'reaction-time' : testId}/?challenge=${token}`;
  };
  ...
  const shareText = `I scored ${scoreLabel} on the BrainBenchmarks ${testName}. Can you beat me?`;
  const url = getChallengeUrl();
  ...
  return (
    <div className="w-full flex flex-col gap-3 mt-4 border-t border-card-border/40 pt-4">
      <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-wider text-center sm:text-left">Share & Challenge Friends</span>
      <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
        <button
          onClick={handleCopy}
          className="flex items-center gap-1.5 px-3 h-8 rounded border border-card-border bg-zinc-900/50 hover:bg-zinc-900 text-zinc-300 hover:text-white text-xs font-mono transition-standard active:scale-95 cursor-pointer"
        >
        ...
        <a
          ...
          className="flex items-center gap-1.5 px-3 h-8 rounded border border-card-border bg-zinc-900/50 hover:bg-zinc-900 text-zinc-300 hover:text-white text-xs font-mono transition-standard active:scale-95"
        >

...
export default function DistributionCurve({
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver((entries) => {
      ...
      if (width > 0)
        setDim({ w: Math.max(300, width), h: Math.max(200, width * 0.45) })
    })
  ...
  const pathD =
    pdfPoints
      ...
      .join("") +
    `L${toX(pdfPoints[pdfPoints.length - 1].x)},${toY(0)}L${toX(pdfPoints[0].x)},${toY(0)}Z`
  ...
  if (userScore != null && distribution.length >= 2) {
    ...
    for (let i = 0; i < sorted.length - 1; i++) {
      if (userScore >= sorted[i].score && userScore <= sorted[i + 1].score) {
        const t =
          (userScore - sorted[i].score) /
          (sorted[i + 1].score - sorted[i].score)
        userPercentile =
          sorted[i].percentile +
          t * (sorted[i + 1].percentile - sorted[i].percentile)
        break
      }
    }
  ...
  const percentilesWithError =
    distribution.length >= 2
      ? (pctLabels
          .map((p) => {
            for (let i = 0; i < distribution.length - 1; i++) {
              if (
                p >= distribution[i].percentile &&
                p <= distribution[i + 1].percentile
              ) {
                const t =
                  (p - distribution[i].percentile) /
                  (distribution[i + 1].percentile - distribution[i].percentile)
                return {
                  pct: p,
                  score:
                    distribution[i].score +
                    t * (distribution[i + 1].score - distribution[i].score),
                }
              }
            }
            return null
  ...
  return (
    <div
      ref={containerRef}
      className="w-full rounded-xl border border-card-border bg-card p-4"
    >
      ...
      >
        ...
        {userPercentile != null && userX != null && (
          <g>
            <rect
              x={Math.min(userX + 10, dim.w - 160)}
              y={padding.top + plotH - 48}
              width="150"
              height="38"
              rx="6"
              fill="var(--color-card)"
            ...
            <text
              ...
              fontFamily="monospace"
            >
              {lowerIsBetter ? "Faster than" : "Better than"}{" "}
              {(100 - userPercentile!).toFixed(1)}% of users
            </text>
          </g>
        )}
